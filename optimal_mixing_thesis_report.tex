...
\title{%
  ...
  \large A Pseudo-Spectral Study of the\\
  Lin--Thiffeault--Doering Strategy
}
...
\begin{abstract}
\noindent
This thesis examines passive-scalar mixing under the instantaneous, or
greedy, velocity field introduced by Lin, Thiffeault and Doering. It measures
the decay of the $H^{-1}$ mix norm and investigates how the observed mixing
rate varies with the length scale of the initial scalar field.

The transport equation is solved on the two-dimensional torus $[0,1]^2$.
A Fourier pseudo-spectral discretisation is combined with adaptive
Dormand--Prince time integration. Four parametrised families of initial
data are implemented, and conservation of the $L^2$, $L^4$ and $L^8$ norms
is used to detect loss of spatial resolution and terminate each run.

For the sine-bump initial data, the computed $H^{-1}$ norm decays
approximately exponentially. If
$\norm{\theta(t)}_{H^{-1}}\approx\norm{\theta_0}_{H^{-1}}e^{-rt}$, the case
$a=0.5$ gives $r\approx0.44$, corresponding to a mixing timescale
$\tau=1/r\approx2.27$. A power-law fit over eight values
$a\in[0.5,0.9375]$ gives $r\propto a^{-1.78}$ at $N=64$. This dependence is
steeper than the $a^{-1}$ scaling associated with the
Iyer--Kiselev--Xu lower bound. At $N=32$, however, the fitted exponent is
approximately $-1.61$. The exponent is therefore not numerically converged,
and the available resolutions do not determine whether its discrepancy
from $-1$ is caused by finite resolution, by the greedy stirring strategy,
or by both effects.

The study provides a reproducible numerical framework for higher-resolution
experiments and for comparisons with finite-horizon optimisation methods.
...
This thesis develops and analyses a pseudo-spectral simulation of passive
scalar mixing on the two-dimensional torus
$\mathbb{T}^2=[0,1]^2$. The solver is a Python implementation of Gautam
Iyer's MATLAB code for the instantaneous steepest-descent strategy derived
by Lin, Thiffeault and Doering (LTD) \cite{LTD2011}.

\subsection{Motivation}

Efficient mixing under a limited stirring budget is relevant to processes
ranging from chemical reactors to environmental transport. In the absence
of molecular diffusion, incompressible advection preserves the scalar's
amplitude distribution. Mixing must therefore be described through the
transfer of scalar variation from large to small spatial scales. Negative
Sobolev norms provide a natural measure of that transfer.

The LTD framework selects, at every instant, the admissible velocity that
produces the steepest decrease of the $H^{-1}$ norm. This local optimisation
problem links the analysis of incompressible transport with constrained
variational methods and Fourier numerics. It also exposes an important
distinction: a velocity that is optimal at the present instant need not be
optimal over a finite time interval.

\subsection{Goals}
\begin{itemize}
  \item Reproduce the results of Gautam Iyer's earlier MATLAB implementation
        of this simulation.
  \item Determine whether the $H^{-1}$ mix norm decays approximately
        exponentially under the Lin--Thiffeault--Doering (LTD) velocity.
...
diffusion. Equation~\eqref{eq:pde} implies that $\theta$ is constant along
particle trajectories $\dot X(t)=u(X(t),t)$. Because a divergence-free
velocity generates a measure-preserving flow map, advection rearranges the
scalar without changing the measure of its level sets. In this
diffusion-free setting, optimal mixing is therefore an optimisation over
incompressible rearrangements.
...
(\nabla\cdot u)\theta^2\dx = 0$). The $H^{-1}$ norm instead weights the
$k$-th mode by $1/|k|^2$. It is therefore dominated by low wavenumbers and
relatively insensitive to fine scales. Moving a fixed amount of $L^2$
energy from low to high wavenumbers reduces $\norm{\theta}_{H^{-1}}$ while
leaving $\norm{\theta}_{L^2}$ unchanged. This is the sense in which the
negative Sobolev norm measures mixing.
...
The final rescaling enforces the enstrophy constraint with equality whenever
$v\ne0$. For the selected descent direction, using the full available
budget gives the largest instantaneous decrease of the squared mix norm.

\subsection{Derivation of the LTD Velocity}
\label{sec:derivation}

Formula~\eqref{eq:ltd} solves a constrained optimisation problem. Its
derivation explains the structure of the five-step algorithm in
Section~\ref{sec:rhs} and identifies the degenerate case discussed in
...
Each RHS evaluation requires ten $N\times N$ FFTs (three forward and seven
inverse transforms, including the intermediate transforms in Steps 1--5).
Since each transform costs $O(N^2\log N)$, the total cost per Runge--Kutta
stage has the same asymptotic order. Doubling $N$ from $32$ to $64$
therefore increases the cost per stage by approximately a factor of
$4$--$5$,
...
spurious energy. The classical remedy in pseudo-spectral codes combines the
$2/3$ truncation rule with $3/2$ zero-padding: retain only the lower
two-thirds of the modes, pad to $3N/2$ points before transforming, multiply
in physical space, and truncate after transforming back. For quadratic
nonlinearities, this removes aliasing from the retained modes at the cost of
larger FFTs.

The implementation does \emph{not} apply this procedure (nor any other
...
\centering
\small
\caption{Initial-data families used by the numerical implementation.}
\label{tab:idata}
...
\subsection{\texorpdfstring{Dependence on Scale Parameter $a$}{Dependence on Scale Parameter a}}
\label{sec:sweep}
...
\subsection{\texorpdfstring{Reproducibility at $N=64$}{Reproducibility at N=64}}
